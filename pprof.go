...
// privatePprofRoutes adds several routes from package `net/http/pprof` to *echo.Group object.
func (s *server) privatePprofRoutes(g *echo.Group) {
	routers := []struct {
		Method  string
		Path    string
	...
	for _, router := range routers {
		...
		case "GET":
			g.GET(router.Path, router.Handler)
		case "POST":
			g.POST(router.Path, router.Handler)
		}
	}
}
